...
import { Fusion2Service, Video1234 } from '../fusion2.service';
...
@Component({
  selector: 'app-mentorupdatecourse',
  standalone: true,
  imports: [CommonModule,FormsModule,ReactiveFormsModule],
  templateUrl: './mentorupdatecourse.component.html',
  styleUrl: './mentorupdatecourse.component.css'
})
export class MentorupdatecourseComponent implements OnInit  {
...
ngOnInit(): void {
  ...
  this.fetchVideos1(this.lessonId);
  this.route.params.subscribe(params => {
    const courseId = +params['courseId']; // Convert to number
    if (courseId) {
      this.courseId = courseId;
      console.log('Course ID from URL:', this.courseId);
  

      // Fetch all necessary data using this courseId
      ...
      // For the fetchVideos1 method, you might need to adjust how you get the lessonId
      // Perhaps fetch it from the first lesson of the course, or handle it differently
      // this.fetchVideos1(lessonId);

    } else {
      console.error('No valid courseId found in URL');
      // Handle the error, maybe redirect to a 404 page or show an error message
      this.router.navigate(['/courses']);
    }
  });

  // Any other initialization that doesn't depend on courseId can go here
  ...
  this.fusionService.createLessonModule(lessonModule, this.courseId)
    .subscribe(
      response => {
        ...
        this.modules[moduleIndex].moduleId = response.id; // Update moduleId with response ID
        console.log('Updated moduleId:', this.modules[moduleIndex].moduleId);
      },
      error => {
...
// Prepare updated module data with the dynamically assigned moduleId
const updatedModuleData = {
  id: module.moduleId, // Use dynamically assigned moduleId
...
if (this.selectedModuleId !== null) {
  this.fusionService.updateLessonByLessonModuleId(this.selectedModuleId, lessonData).subscribe(
    (response) => {
      console.log('Lesson updated successfully:', response);
  ...
  console.error('Selected module ID is null');
}
}
...
// For example, open a dialog for editing the video URL
  ...
  fetchVideos123(): void {
    this.fusion2service.getVideosByCourse(this.courseId).subscribe(
      (data: Video1234[]) => {
        this.videos123 = data;
      },
      error => {
